...
public class BattleScript : MonoBehaviour
{
    ...
    private bool askedForPOneCard = false;
    private bool askedForPTwoCard = false;
    ...
    private void Update()
    {
        if (playerOneChampion == null && !askedForPOneCard)
        {
            ChampNetManager.RequestMissingChampion(1);
            askedForPOneCard = true;
        }
        else if (playerTwoChampion == null && !askedForPTwoCard)
        {
            ChampNetManager.RequestMissingChampion(2);
            askedForPTwoCard = true;
        }
        ...
        if (playerNum == 1)
        {
            if (playerOneChampion != null && playerOneChampion.GetCardInfo().health <= 0)
            {
        ...
        {
            if (playerTwoChampion != null && playerTwoChampion.GetCardInfo().health <= 0)
            {
                //For now just override the dead champion
    ...
    public void PlayerOneAttack()
    {
        ...
        GameObject GO = Instantiate(damagePop, playerTwoChampion.transform);
        GO.GetComponent<RectTransform>().localPosition = new Vector2(0f, -92);
        GO.GetComponent<DamagePop>().Set(finalDamage);
    ...
    public void PlayerTwoAttack()
    {
        ...
        GameObject GO = Instantiate(damagePop, playerOneChampion.transform);
        GO.GetComponent<RectTransform>().localPosition = new Vector2(0f, -92);
        GO.GetComponent<DamagePop>().Set(finalDamage);
        GO.transform.SetParent(canvas);
